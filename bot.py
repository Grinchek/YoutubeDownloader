...
SOURCE_TAG = "@YouTubevideoDownloaderNewbot"  # лише для підпису в caption
...
def ydl_probe_opts(out_dir: str, player_client: str) -> dict:
    return {
        **_common_opts(out_dir, player_client),
        "skip_download": True,  # лише метадані
    }
...
def probe_info(url: str, out_dir: str) -> Tuple[Dict[str, Any], str]:
    """
    Спочатку пробуємо 'web' (найповніше), якщо падає — 'android'.
    Повертаємо (info, used_client).
    """
...
async def on_format_selected(call: CallbackQuery):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        caption = f"Завантажено з: {SOURCE_TAG}\n<b>Формат:</b> {note}"
        try:
